...
function getSpecificProj(){
  $con = mysqli_connect("localhost","root","","posts");
  $srch=$_GET['id'];
  $query="SELECT * FROM `projecttable2` where id ='$srch'";
  $query2="SELECT * FROM `projectcomments` where postid ='$srch' order by dates desc";
  //$query = "select * from posttable order by id desc";
  $result=mysqli_query($con,$query);
  while($row=mysqli_fetch_array($result)){
    echo '
    ...
    <div class="searchbar2">
    <form action="addComment_proj.php?id='.$row["id"].'" method="POST" id="form1">
    	<br>
      <p class="posttitle" style="">Add a comment</p>
      <input type="text" class="form-control" name="text" id="title">
      <br>
